---
project: taskx
doc_kind: roadmap_step
layer: 1
track: M1
step_id: M1.4b
status: planned
historical_order: 8
depends_on:
  - M1.4
evidence:
added_at: 2026-02-18 18:56:56 +0100
completed_at:
updated_at: 2026-02-18 18:56:56 +0100
---

# M1.4b — Dev debug mirror for rendered block state (follow-up, implicit) 🟡


Intent:

- Improve debugging feedback loops by persisting the rendered block state to disk
  during development.

Constraints:

- The mirror is debug-only and must not change ranking/recommendation policy.
- The mirror is gated by a dedicated boolean flag.
- Default behavior: enabled in dev mode.
- Mirror output path for current loop: `plugins/taskx/temp/task_feed.md`.

Implementation note:

- Write a sanitized mirror file of block content to local disk when the debug
  mirror flag is `true`.
- The mirror file is considered valid when:
  - a step-task change is made,
  - TaskX reruns,
  - `task_feed.md` is present/updated,
  - and the mirrored feed content matches the feed shown in Obsidian.
